...
class DHT11:
    ...
    def read(self):
        GPIO.pinMode(self.__pin, self.OUT)

        # send initial high
        self.__send_and_sleep(self.HIGH, 0.05)

        # pull down to low
        self.__send_and_sleep(self.LOW, 0.02)

        # change to input using pull up
        GPIO.pinMode(self.__pin, self.IN)
    ...
    def __send_and_sleep(self, output, sleep):
        GPIO.digitalWrite(self.__pin, output)
        time.sleep(sleep)
    ...
    def __collect_input(self):
        ...
        data = []
        while True:
            current = GPIO.digitalRead(self.__pin)
            data.append(current)
            if last != current:
                unchanged_count = 0
                last = current
